...
1. **The Scraper:** As you scroll, Iris privately reads the DOM of your feed (X, Instagram, etc.).
2. **The Scrubber:** Before any data leaves your machine, an on-device, WebGPU-accelerated AI scrubs the text and images to remove Personally Identifiable Information (PII), real names, and private contacts.
3. **The Mesh:** Your sanitized feed is broadcasted to a Peer-to-Peer (P2P) network. No central servers. No corporate databases.
4. **The Mask:** When you activate "Perspective Swap," Iris hides your actual feed and injects a custom interface displaying a live feed from another random user on the mesh.

## 🕵️ Stealth & Security (The Ghost Protocol)
Scraping data is against the Terms of Service of major social platforms. To protect the user, Iris operates entirely as a **passive ghost** using three core principles to evade anti-bot detection:
* **The Piggyback Method:** Iris does not actively crawl or automatically scroll the page. It relies entirely on the `MutationObserver` API to passively read the HTML as the user naturally scrolls. To the platform's behavioral algorithms, it simply looks like human screen reading.
* **Air-Gapped Networking:** Broadcasted feeds do not utilize the host platform's servers. Feed sharing occurs over an entirely separate, decentralized WebRTC browser-to-browser mesh.
* **Client-Side Illusions:** Swapping perspectives does not send irregular data requests to the host platform. Iris simply uses local CSS and DOM manipulation to hide the host's timeline and paint a custom UI overlay on top of it.
...
- [x] Implement `MutationObserver` to capture infinite-scroll feed data in real-time.
...
4. Click **"Load unpacked"** and select the `iris-extension` folder.
5. Navigate to X (Twitter) or Instagram, click the Iris icon in your toolbar, and click **Open Iris**.
